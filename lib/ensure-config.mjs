...
// token "openclaw" (owner's decision, 2026-09-12 — changed back from "yuai").
//
...
// now the fixed value "openclaw" again — note this trades away the LAN-hardening
...
//
// B-13 (2026-09-14): same self-heal for `logging.file`. Without it the kernel
// writes its rolling log into the HOST temp dir (/tmp/openclaw, on Windows
// %TEMP%\openclaw) — breaking the zero-trace promise — and none of the
// /api/logs candidates in config-server/server.js match it, so the log panel
// is permanently "not found". The kernel uses `logging.file` verbatim and
// size-rotates it (max 5 files), and the portable value
// <pkg>/data/logs/gateway.log is probe candidate #1, so one absolute path
// fixes both. A logging.file that already points inside the package's
// data/ dir is the user's choice and is never touched; anything else
// (missing, relative, or stale after a USB drive-letter change) is
...
// real token.
// "openclaw" is the target value (2026-09-12) and must read as valid; "yuai"
// is the 2026-09-10 value and is treated as a placeholder so legacy configs
// get rewritten to the unified token on the next launch.
const PLACEHOLDER_TOKENS = new Set(["", "yuai", "__GENERATED_AT_FIRST_RUN__"]);
const FIXED_TOKEN = "openclaw";
...
// Every caller (9 launchers + maintain.sh) passes a config that lives at
// <pkg>/data/.openclaw/openclaw.json, so the shared data dir is two levels up.
// Deriving it from the argument (instead of __dirname) keeps the log file
// next to whatever state dir the caller actually uses.
const DATA_DIR = path.dirname(path.dirname(path.resolve(configPath)));
const PORTABLE_LOG_FILE = path.join(DATA_DIR, "logs", "gateway.log");

function loggingFileIsPortable(config) {
  ...
  // which path.isAbsolute below catches.
  const rel = path.relative(DATA_DIR, path.resolve(file.trim()));
  return rel !== "" && !rel.startsWith("..") && !path.isAbsolute(rel);
}

function applyPortableLogging(config) {
  if (!config.logging || typeof config.logging !== "object" || Array.isArray(config.logging)) {
    config.logging = {};
  }
  config.logging.file = PORTABLE_LOG_FILE;
  fs.mkdirSync(path.dirname(PORTABLE_LOG_FILE), { recursive: true });
}
...
try {
  ...
  if (fs.existsSync(configPath) && !force) {
    ...
    const loggingUsable = loggingFileIsPortable(config);
    if (authUsable && loggingUsable) process.exit(0);

    if (!authUsable) applyFixedAuth(config);
    if (!loggingUsable) applyPortableLogging(config);
    writeConfig(configPath, config);
    if (!authUsable) {
      console.log(`  [ok] 网关口令缺失，已补写固定值 "${FIXED_TOKEN}"`);
    }
    if (!loggingUsable) {
      console.log("  [ok] 日志路径缺失或失效，已改写为包内 data/logs/gateway.log");
    }
    process.exit(0);
  }
  ...
  applyPortableLogging(config);
  writeConfig(configPath, config);
  console.log(`  [ok] 已生成配置，网关口令为固定值 "${FIXED_TOKEN}"`);
  console.log("  [ok] 网关日志固定写入包内 data/logs/gateway.log");
} catch (error) {
