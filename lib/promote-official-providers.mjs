 ...
 *
 * ⚠️ 何时必须重跑本脚本：
 *   任何一次 npm 生命周期重跑都会把提升的插件删光 —— 重装依赖、升级
 *   openclaw、或网关启动时检测到 lifecycle-pending 标记。原因是 openclaw 的
 *   postinstall 会拿原厂发货清单比对磁盘（见文末"关键第二步"）。
 *   典型事故（2026-09-04 手工把内核从 2026.8.2 升到 2026.9.1）：npm install
 *   成功后 19 个 provider 从 `plugins list` 全部消失（79 → 59），配置中心的
 *   `getOfficialModelCatalog()` 随之丢掉这些 provider 的官方模型行，退回写合成
 *   `models[]` —— 直接复现"上下文被砍 5 倍 / 输出限 47 倍 / 成本记 0"那个 P0。
 *   重跑本脚本即完全恢复（实测 19/19，插件清单与升级前零差异）。
 *
 * 用法：
 *   node promote-official-providers.mjs [CORE_DIR]
 *   CORE_DIR 可选：不传时自动向上寻找含 app/core 的 portable 根目录。
 *   （发布包把本脚本放在 system/lib/，开发树在 lib/，两种布局都能自动识别；
 *     建议 setup 脚本仍显式传参，意图更明确。）
 *
...
// marker 文件名。config-server/server.js 的 discoverBuiltinProviderIds() 用它
// 区分 "promote 的官方插件"（可配置中心编辑）与 "openclaw 原厂 stock 扩展"
// （官方托管、走官方 UI）。改名时两处必须同步。
const PROMOTE_MARKER = '.portable-promoted';

function resolveCoreDir() {
  if (process.argv[2]) return path.resolve(process.argv[2]);
  // 自动向上寻找含 app/core 的 portable 根目录。
  //
  // 不能简单地取 `__dirname/..`：脚本在两种布局下都可能出现——
  //   开发树  <portable>/lib/            → `..` = <portable>            ✅
  //   发布包  <portable>/system/lib/     → `..` = <portable>/system     ❌
  // 发布包布局下旧实现会算出不存在的 <portable>/system/app/core，
  // 而后面的 `if (!fs.existsSync(openclawPkg))` 只是 exit(0) 静默跳过——
  // 手工重跑时极易误以为"已经 promote 过了"。改为逐级上溯，两种布局都对。
  let dir = __dirname;
  for (let i = 0; i < 6; i += 1) {
    const candidate = path.join(dir, 'app', 'core');
    if (fs.existsSync(candidate)) return candidate;
    const parent = path.dirname(dir);
    if (parent === dir) break;
    dir = parent;
  }
  // 兜底：保持旧行为，让下面的 existsSync 检查给出明确的 skip 提示
  return path.join(path.resolve(__dirname, '..'), 'app', 'core');
}
...
for (const id of PROVIDER_IDS) {
  ...
  // marker：向配置中心声明"这个 bundled 插件是便携包 promote 的，不是 openclaw
  // 原厂 stock"。配置中心的"官方托管"判定据此把它排除——这样 consent 打通
  // （本脚本的目的）与配置中心可写权（方向 A 的管辖边界）可以同时成立。
  // 详见可行性分析 §13.7-A 的 A3 方案。
  // 必须写在 collectFilesRecursively 收集之前，marker 才会被收进发货清单、
  // 不被 postinstall prune 删掉。
  if (entryOk) {
    fs.writeFileSync(
      path.join(dst, PROMOTE_MARKER),
      'promoted by openclaw-portable promote-official-providers.mjs\n' +
      'config-server treats directories carrying this marker as user-configurable\n' +
      '(NOT official-managed), while openclaw itself still sees them as bundled.\n',
      'utf8'
    );
  }
}
